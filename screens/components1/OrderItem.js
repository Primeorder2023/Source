...
var imagename=require("./images/voice-in.png");
...
const shareOptions = {
  ...
  if(this.state.From=='HISTORY'|| TYPE=='RETURN'|| this.state.From=='RP'||commonData.getContext()=='RP'){
    ...
    console.log("my return order raaa",itmArray);
...
this.forceUpdate()
  ...
  joinData = () => {
    ...
    if (found) {
      ...
      if ((this.state.qty == 0 || this.state.qty == -1)) {
         this.state.qty = 1;
        this.state.ItemArray.push({id:id, itemid: this.state.itemID,tax:tax, description: this.state.description, price: price, qty: this.state.qty, imgsrc: this.state.itemImage,weight:weight,stock:hand});
       }
  ...
  calculaterunningTotals() {
   ...
   this.state.gstvalue=0;
    for (var i = 0; i < ItemVAl; i++) {
      Qtyval = Qtyval + Number(this.state.arrayHolder[i].qty)
      if (this.state.arrayHolder[i].price != null)
      var itemprice= Number(this.state.arrayHolder[i].qty) * Number(this.state.arrayHolder[i].price);
      var taxamount= Number(this.state.arrayHolder[i].tax)*Number(itemprice)/100;
      this.state.gstvalue=this.state.gstvalue+taxamount;
      itemprice=itemprice+ taxamount;
      console.log(this.state.gstvalue,"this.state.arrayHolder[i].qty",this.state.arrayHolder[i])
        PriceVal = Number(PriceVal) + itemprice;
    }
    console.log(PriceVal,":",itemprice,":",taxamount)
 ...
 onSpeechResults = (e) => {
  ...
  var array2= e.value;
    for(var i=0;i<e.value.length;i++){
      var strarray = e.value[i].split(" ");
      for(var j=0;j<strarray.length;j++){
    this.searchitemwithdescription(strarray[j]);
      }
    }
...
startRecognizing = async () => {
  ...
  imagename=require("./images/voice-search.gif");
  try {
 ...
 stopRecognizing = async () => {
  ...
  try {
     imagename=require("./images/voice-in.png");
    await Voice.stop();
  } catch (e) {
    //eslint-disable-next-line
    console.error(e);
...
fetch("http://143.110.178.47/OrdoCRM7126/uploadFile.php", requestOptions)
  ...
  this.props.navigation.navigate('OrderSent',{"TYPE":TYPE,"doc_id":result.document_id})
...
sendReturnFunction=()=>{
  var that =this;
  console.log("return parent",that.state.arrayHolder);
  let lineitems=[];
  var orderid=""
  for(var i=0;i<that.state.arrayHolder.length;i++){
    orderid=that.state.arrayHolder[0].orderid;
    lineitems.push({id:that.state.arrayHolder[i].rid,qty:that.state.arrayHolder[i].qty});
  }
  var myHeaders = new Headers();
  myHeaders.append("Content-Type", "application/json");
  ...
  var raw = JSON.stringify({
    "__return_products_quotes_id_array__": lineitems,
    "__quotes_id__": orderid
  });
  console.log("my return raw",raw);
  var requestOptions = {
    method: 'POST',
    headers: myHeaders,
    body: raw,
  ...
  fetch("https://dev.ordo.primesophic.com/set_return_order.php", requestOptions)
    .then(response => response.json())
    .then(result => {console.log("result return order",result);
      that.updatethestatusReturn(result.return_id);
    })
    ...
    if (this.state.showsubstitute==true) {
      return (
          <View style={{ flex: 1, alignItems: 'center',  alignItems: 'center', justifyContent: 'center' ,backgroundColor: 'rgba(100, 100, 100, 0.9)'}}>
             <TouchableOpacity style={{width:40,height:40,marginHorizontal:width-60}} onPress={()=>{this.setState({showsubstitute:false})}}>
             <Image
            style={styles.closeButton}
            source={require("./images/close_btn.png")}
            />
             </TouchableOpacity>
        ...
        <TouchableOpacity onPress={()=>this.searchitemwithdescription("")} style={{ marginTop: 0,marginHorizontal:5}}>
                <Card 
              style={[styles.signIn,styles.shadowProp]}>
            ...
            <TouchableOpacity   style={{width:40,height:40,marginTop:-5,borderRadius:20}}   onPress={this.startRecognizing}>
        </TouchableOpacity>
           </View>
      );
  }
...
<SafeAreaView style={styles.container}>
    <View style={{alignSelf:'center',height:height-90,width:width-10}}>
      ...
      <Text style={{color:'#34495A',fontSize:11,borderBottomColor:'#7A7F85',fontFamily:'Lato-Bold',fontWeight:"500",marginHorizontal:5}}>{commonData.getActiveCustName()} </Text>
      <Text style={{color:'#34495A',fontSize:10,borderBottomColor:'#7A7F85',fontFamily:'Lato-Regular',fontWeight:"500",marginHorizontal:5}}>{commonData.getActiveAdress()}</Text>
      <Text style={{color:'#34495A',fontSize:10,borderBottomColor:'#7A7F85',fontFamily:'Lato-Regular',fontWeight:"500",marginHorizontal:5}}>GSTIN: {Configuration.GSTIN}</Text>
...
<View style={{height:130}}>
      ...
      <Text style={{color:'#000000',width:width-30,fontFamily:'Lato-Regular',fontSize:11,textAlign:'right'}}>GST : ₹{gst}</Text>
      <Text style={{color:'#000000',width:width-30,fontFamily:'Lato-Regular',fontSize:11,textAlign:'right'}}>Saving : ₹{savings}</Text>
      <Text style={{color:'#000000', width:width-30,textAlign:'right',fontFamily:'Lato-Bold',fontSize:12,textAlign:'right'}}>GRAND TOTAL : {Number(grandtotal).toFixed(2)}</Text>
      </View>
...
<View style={{alignSelf:'center',height:height-90,width:width-10}}>
      ...
      <View style={{ flexDirection: 'row',alignSelf:'center', marginTop:10 , width:width-20,backgroundColor:'#ffffff'}} >
          ...
          <Image transition={false}  source={require('../components1/images/OrDo.png')} style={{ marginTop: 10, marginHorizontal: width-320 ,width: 100,height:40,alignSelf:'center' }} > 
          </Image> 
...
<Card style={{ height: 40, width: 120, backgroundColor: 'white',alignSelf:'center',justifyContent:'center',borderRadius:8}}>
             <TouchableOpacity  style={{height: 40,width:120,alignItems:'center',justifyContent:'center'}}  onPress={()=>this.save()}>
            
            <Text style  ={{fontFamily:"Lato-Regular",textAlign:'center',color: 'green',
...
<View style={{width:(width-80)/2,height:60}}>
<Card style={{ height: 40, width: 120, backgroundColor: 'white',alignSelf:'center',justifyContent:'center',borderRadius:8}}>
             <TouchableOpacity style={{height: 40,width:120,alignItems:'center',justifyContent:'center'}}   onPress={()=>this.cancel()}>
            
            <Text style  ={{fontFamily:"Lato-Regular",textAlign:'center',color: 'red',
    // fontFamily:"Lato-Bold",
    textAlignVertical:'center',
    // fontWeight: 'bold',
          ...
          <View style={{ backgroundColor: '#FFFFFF',flex:1}}>
            <View style={{ flexDirection: 'row', backgroundColor: '#FFFFFF' ,marginHorizontal:5,alignContent:'center',justifyContent:'center'}}>
              ...
              <View style={{ marginTop:10,alignSelf:'center', backgroundColor: '#FFFFFF',width:width-10,height: 50, flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
                ...
                <TouchableOpacity  onPress={() => commonData.isOrderOpen==true ? this.open_QR_Code_Scanner() : Alert.alert('There is no Active order')} style={{ marginTop: 10,marginHorizontal:5,flex:0.15}}>
...
<View style={styles.flatliststyle1}>  
  <View style={{ flexDirection: "row",justifyContent:'center',alignItems:'center', backgroundColor:'white',width:width-10,alignSelf:'center' ,height:40}} >
   <Text style={{color:'#34495A',borderBottomColor:'#7A7F85',fontWeight:'500',textAlign:'left',fontFamily:'Lato-Bold',fontSize:10,height:40, width:width/2.5-20}}>{item.description}</Text>
   <Text style={{color:'#34495A',borderBottomColor:'#7A7F85',fontWeight:'500',textAlign:'center',fontFamily:'Lato-Bold',fontSize:12,width:width/4.5}}>{Number(item.qty)}</Text>
   <Text style={{color:'#34495A',borderBottomColor:'#7A7F85',fontWeight:'900',textAlign:'center',fontFamily:'Lato-Bold',fontSize:12,width:width/3.0}}>₹{Number(item.price)+Number(item.price)*Number(item.tax)/100}</Text>
  </View> 
</View>
